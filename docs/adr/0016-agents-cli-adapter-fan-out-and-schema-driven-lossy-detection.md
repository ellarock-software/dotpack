...
- **Per-kind canonical writer for file-drop write-once**: `umbrellaFactories[{umbrella}].writers[{kind}]` names ONE sub-adapter's HostID per kind. For `agents-cli + skill → codex` (codex's `AgentsHome/skills/{name}/SKILL.md` per developers.openai.com/codex/skills; gemini-cli reads the same path per `schema/skill.yaml` ecosystem_notes). Kinds absent from `writers` are explicitly unsupported under the umbrella — `UmbrellaInstaller.Install` returns "kind not supported under umbrella" rather than silently picking a default sub-adapter. Agent kind is the canonical absent case today: no documented cross-host convergence path, so the umbrella refuses (gemini-cli writes to `GeminiHome/agents/`; codex has no native agent dir).
...
- **Config-fragment fan-out**: today's `writers map[resource.Kind]adapter.Adapter` declares ONE canonical writer per kind, which is correct for file-drop write-once. When hook + mcp-server land (§5–§7), the per-kind value widens from one writer to an ordered list (each sub-adapter writes to its OWN config file, plans are merged into one record's `MergedKeys`). The `(subAdapters, label)` shape on `UmbrellaInstaller` doesn't change; only the per-kind dispatch inside `Install` does. The §9 un-merge-keys story for uninstall moves Reader.Uninstall → Installer.Uninstall as Card #3's docstring documents; the umbrella un-merge-keys path is the equivalent migration for `UmbrellaInstaller.Uninstall` (does not exist today; Reader handles all uninstalls).
