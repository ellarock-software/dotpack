...
- Existing behavior for claude-code, gemini-cli, antigravity-cli, codex,
  opencode, hermes, and agents-cli is preserved (the JSON/TOML/YAML
  backends wrap the prior functions; the registry reproduces the prior
  validation).
...
- **Pi adapter** is named backlog; the onboarding path is the one in §1, only
  per-host paths need confirming.
