...
Per [ADR-0001](./0001-empirically-derived-schema-via-corpus-survey.md). Fifth kind surveyed; **re-surveyed with Codex corpus after `scripts/survey.sh` TOML extraction landed** (task #14). Corpus is now 4 Claude `.claude/settings.json` + 1 Gemini `.gemini/settings.json` + 2 Codex `.codex/config.toml` = 7 files, 66 bindings, 67 hook-spec leaves.
