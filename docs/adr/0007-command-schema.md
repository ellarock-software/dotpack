...
Per [ADR-0001](./0001-empirically-derived-schema-via-corpus-survey.md). Third kind surveyed. The `command` kind has a known cross-ecosystem format split flagged in the original handoff: Claude Code stores commands as markdown with optional YAML frontmatter under `.claude/commands/*.md`; Gemini CLI stores commands as TOML under `.gemini/commands/*.toml`. The corpus deliberately includes both.
