...
func TestHostKeepsExtension_HermesSkillFields_AreNativeOnHermes(t *testing.T) {
	s, err := schema.Load(resource.KindSkill)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	for _, field := range []string{"platforms", "required_environment_variables", "required_credential_files"} {
		if !s.HostKeepsExtension("hermes", field) {
			t.Errorf("HostKeepsExtension(hermes, %s) = false; want true", field)
		}
		if s.HostKeepsExtension("codex", field) {
			t.Errorf("HostKeepsExtension(codex, %s) = true; want false", field)
		}
	}
}
