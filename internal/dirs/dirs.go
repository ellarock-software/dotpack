...
type Dirs struct {
	...
	// HermesHome is the root of Hermes Agent's user config, e.g.
	// ~/.hermes. The hermes adapter writes user-scope skills to
	// HermesHome/skills/<name>/SKILL.md, user-scope memory to
	// HermesHome/SOUL.md, and merges mcp-server + hook installs into
...
// DOTPACK_AGENTS_HOME / DOTPACK_CODEX_HOME / DOTPACK_HERMES_HOME /
...
//     DOTPACK_CODEX_HOME / DOTPACK_HERMES_HOME / DOTPACK_DOTPACK_HOME are
//     dotpack-managed WRITE targets; install MkdirAll's their trees on
//     first use, so FromEnv must tolerate a not-yet-existing path.
//     Normalise to absolute, do not stat.
//
...
func FromEnv() (Dirs, error) {
	d := Dirs{
		...
		HermesHome:      os.Getenv("DOTPACK_HERMES_HOME"),
		DotpackHome:     os.Getenv("DOTPACK_DOTPACK_HOME"),
		ProjectHome:     os.Getenv("DOTPACK_PROJECT_HOME"),
	}

	if d.HomeDir == "" || d.ClaudeHome == "" || d.GeminiHome == "" || d.AntigravityHome == "" || d.AgentsHome == "" || d.OpenCodeHome == "" || d.CodexHome == "" || d.HermesHome == "" || d.DotpackHome == "" {
		...
		if d.HermesHome == "" {
			d.HermesHome = filepath.Join(home, ".hermes")
		}
		if d.DotpackHome == "" {
	...
	if abs, err := filepath.Abs(d.HermesHome); err == nil {
		d.HermesHome = abs
	} else {
		return Dirs{}, fmt.Errorf("DOTPACK_HERMES_HOME=%q: resolve abs: %w", d.HermesHome, err)
	}
