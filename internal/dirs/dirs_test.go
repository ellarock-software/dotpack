...
func TestFromEnv_RelativeHomeFromDeletedCWDReturnsAbsError(t *testing.T) {
	...
	t.Setenv("DOTPACK_HERMES_HOME", t.TempDir())
	t.Setenv("DOTPACK_DOTPACK_HOME", t.TempDir())
	t.Setenv("DOTPACK_PROJECT_HOME", t.TempDir())
...
func TestFromEnv_HermesHome_RelativeEnvIsResolvedToAbsolute(t *testing.T) {
	wantParent := t.TempDir()
	rel := "hermes-cfg"
	t.Chdir(wantParent)
	t.Setenv("DOTPACK_CLAUDE_HOME", t.TempDir())
	t.Setenv("DOTPACK_HERMES_HOME", rel)
	t.Setenv("DOTPACK_DOTPACK_HOME", t.TempDir())
	...
	if !filepath.IsAbs(d.HermesHome) {
		t.Errorf("HermesHome must be absolute after FromEnv normalises env input; got %q", d.HermesHome)
	}
	wantAbs := filepath.Join(wantParent, rel)
	if filepath.Clean(d.HermesHome) != filepath.Clean(wantAbs) {
		t.Errorf("HermesHome: got %q, want %q", d.HermesHome, wantAbs)
	}
}

func TestFromEnv_HermesHome_NonexistentEnvDoesNotError(t *testing.T) {
	parent := t.TempDir()
	t.Setenv("DOTPACK_CLAUDE_HOME", t.TempDir())
	t.Setenv("DOTPACK_HERMES_HOME", filepath.Join(parent, "does-not-exist-yet"))
	t.Setenv("DOTPACK_DOTPACK_HOME", t.TempDir())
	...
	if !filepath.IsAbs(d.HermesHome) {
		t.Errorf("HermesHome must be absolute; got %q", d.HermesHome)
	}
}
