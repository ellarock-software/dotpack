	...
	for _, ev := range h.Events {
		if _, ok := canonicalHookEvents[ev.Event]; !ok {
			errs = append(errs, ValidationError{
				...
				Message: "event name not in schema/hook.yaml canonical_event_names (most likely a typo — Claude+Codex use PascalCase identity; Gemini's BeforeTool/AfterTool is translator-side)",
			})
			continue
		}
