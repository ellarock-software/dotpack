...
func collectSyncBackSkillPrimaryFiles(items []orchestrator.InventoryItem) []string {
	seen := map[string]struct{}{}
	files := make([]string, 0)
	for _, item := range items {
		if item.Kind != string(resource.KindSkill) {
			continue
		}
		if item.Status != orchestrator.InventoryDrifted && item.Status != orchestrator.InventoryForeignUntracked && item.Status != orchestrator.InventoryCanonicalUntracked {
			continue
		}
		primary := filepath.Join(item.TargetDir, "SKILL.md")
		if item.TargetDir == "" {
			primary = item.Path
		}
