...
func TestPlanCoversFileDropAndConfigFragments(t *testing.T) {
	d := dirs.Dirs{GeminiHome: t.TempDir(), ProjectHome: t.TempDir()}
	a := New(d)
	...
	cases := []struct {
		...
		{"skill", &resource.Skill{Name: "skill", Description: "d", Body: "b"}, adapter.ScopeUser, filepath.Join(d.GeminiHome, "skills", "skill", "SKILL.md"), ""},
		{"agent", &resource.Agent{Name: "agent", Description: "d", Model: "m", Tools: []string{"Read"}, Body: "body"}, adapter.ScopeProject, filepath.Join(d.ProjectHome, ".gemini", "agents", "agent.md"), ""},
		{"rule", &resource.Rule{Name: "rule", Body: "rule body"}, adapter.ScopeProject, filepath.Join(d.ProjectHome, ".gemini", "rules", "rule.md"), ""},
		{"command", &resource.Command{Name: "cmd", Description: "d", Prompt: "run"}, adapter.ScopeProject, filepath.Join(d.ProjectHome, ".gemini", "commands", "cmd.toml"), ""},
		{"memory", (&resource.Memory{Body: "remember"}).WithName("GEMINI.md"), adapter.ScopeProject, filepath.Join(d.ProjectHome, "GEMINI.md"), ""},
		{"mcp-user", mcpFixture(), adapter.ScopeUser, "", filepath.Join(d.GeminiHome, "settings.json")},
		{"mcp-project", mcpFixture(), adapter.ScopeProject, "", filepath.Join(d.ProjectHome, ".gemini", "settings.json")},
		{"hook-user", hookFixture(), adapter.ScopeUser, "", filepath.Join(d.GeminiHome, "settings.json")},
		{"hook-project", hookFixture(), adapter.ScopeProject, "", filepath.Join(d.ProjectHome, ".gemini", "settings.json")},
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantMerge != "" {
				...
				if tc.name == "hook-user" && plan.MergedKeys[0].Path != "$.hooks.BeforeTool" {
					t.Fatalf("gemini hook event not rewritten: %+v", plan.MergedKeys[0])
				}
			}
		})
	}
...
func TestPlanUnsupportedKindAndEmitErrors(t *testing.T) {
	a := New(dirs.Dirs{GeminiHome: t.TempDir(), ProjectHome: t.TempDir()})
	...
	if _, err := emitMCPServerGemini(&resource.Skill{Name: "no"}); err == nil {
		t.Fatal("emitMCPServerGemini should reject wrong resource type")
	}
	if _, err := emitHookGemini(&resource.Skill{Name: "no"}); err == nil {
		t.Fatal("emitHookGemini should reject wrong resource type")
	}
	if _, err := emitHookGemini(&resource.Hook{Name: "empty"}); err == nil || !strings.Contains(err.Error(), "no events") {
		t.Fatalf("empty hook error = %v", err)
	}
}

func TestGeminiHookEventAndGeneratedNames(t *testing.T) {
	h := &resource.Hook{
		Name: "guard",
		Events: []resource.EventBinding{
	...
	frags, err := emitHookGemini(h)
	if err != nil {
		t.Fatalf("emitHookGemini: %v", err)
	}
	if len(frags) != 2 {
...
func TestGeminiMCPAndHookExtensionEdgeBranches(t *testing.T) {
	...
	frags, err := emitMCPServerGemini(mcp)
	if err != nil {
		t.Fatalf("emitMCPServerGemini: %v", err)
	}
...
func TestPlanMissingRootsReturnHostSpecificErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"skill-user", &resource.Skill{Name: "s", Description: "d", Body: "b"}, adapter.ScopeUser, "GeminiHome"},
		{"skill-project", &resource.Skill{Name: "s", Description: "d", Body: "b"}, adapter.ScopeProject, "ProjectHome"},
		{"mcp-user", mcpFixture(), adapter.ScopeUser, "GeminiHome"},
		{"mcp-project", mcpFixture(), adapter.ScopeProject, "ProjectHome"},
		{"hook-user", hookFixture(), adapter.ScopeUser, "GeminiHome"},
