...
const (
	...
	// FormatYAML: yaml.v3 read-modify-write. Used by Hermes'
	// ~/.hermes/config.yaml.
...
func New(d dirs.Dirs, p Policy) *Adapter {
	...
	for k, cfg := range p.Kinds {
		...
		default:
			panic(fmt.Sprintf("configfrag: %s: Kinds[%q].Format = %q is unknown (use FormatJSON, FormatTOML, or FormatYAML)", p.HostID, k, cfg.Format))
		}
	}
