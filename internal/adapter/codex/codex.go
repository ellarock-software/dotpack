...
func emitMCPServerCodex(r resource.Resource) ([]configfrag.MergedFragment, error) {
	...
	for k, v := range m.Extensions() {
		...
		if k == "tools" && !isCodexMCPToolsPolicy(v) {
			return nil, fmt.Errorf("emit mcp-server: %s.tools is not a Codex per-tool policy map", m.Name)
		}
		value[k] = v
	}
...
func isCodexMCPToolsPolicy(v any) bool {
	...
	for key, val := range m {
		switch key {
		case "include", "exclude", "resources", "prompts":
			return false
		}
